...
container-up:
	# Start a local version of the web app that uses the SurveyStream dev database
...
container-up-stg:
	...
	--region ap-south-1 \
	--config-name surveystream-web-app-config-frontend
	...
	--project-name app \
	--cluster-config surveystream-web-app-config-frontend \
...
container-prod-new:
	...
	--default-launch-type EC2 \
	--region ap-south-1 \
	--config-name surveystream-web-app-config
	...
	--project-name surveystream-web-app \
	--cluster-config surveystream-web-app-config \
...
image-prod:
	...
	@docker tag $(BACKEND_NAME):$(VERSION) 678681925278.dkr.ecr.ap-south-1.amazonaws.com/surveystream-web-app:backend
	@$$(aws ecr get-login --no-include-email --region ap-south-1)
	@docker push 678681925278.dkr.ecr.ap-south-1.amazonaws.com/surveystream-web-app:backend

	@docker build -f Dockerfile.client --rm -t $(FRONTEND_NAME):$(VERSION) . 
	@docker tag $(FRONTEND_NAME):$(VERSION) 678681925278.dkr.ecr.ap-south-1.amazonaws.com/surveystream-web-app:frontend
	@$$(aws ecr get-login --no-include-email --region ap-south-1)
	@docker push 678681925278.dkr.ecr.ap-south-1.amazonaws.com/surveystream-web-app:frontend

container-up-prod:
	# Configure ecs-cli options
	@ecs-cli configure --cluster surveystream-web-app-cluster \
	--default-launch-type EC2 \
	--region ap-south-1 \
	--config-name surveystream-web-app-config
	...
	--project-name surveystream-web-app \
	--cluster-config surveystream-web-app-config \
	--task-role-arn arn:aws:iam::678681925278:role/surveystream-web-app-task-role \
	service up \
	--target-group-arn arn:aws:elasticloadbalancing:ap-south-1:678681925278:targetgroup/surveystream-web-app-tg-443/440079da841258e5 \
	--container-name client \
	--container-port 80 \
	--create-log-groups \
	--deployment-min-healthy-percent 0
